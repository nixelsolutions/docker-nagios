...
RUN mkdir -p ${NAGIOS_HOME}/etc/conf.d && mkdir -p ${NAGIOS_HOME}/etc/monitor && ln -s /usr/share/snmp/mibs ${NAGIOS_HOME}/libexec/mibs && mkdir -p ${NAGIOS_HOME}/.ssh && chown nagios:nagios ${NAGIOS_HOME}/.ssh
